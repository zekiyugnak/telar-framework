---
id: web-ui-ux-specialist
model: sonnet
category: agent
tags: [ui, ux, web, responsive, shadcn, radix, tailwind, forms, i18n, rtl, a11y, states]
capabilities:
  ...
  - Tailwind v4 token-driven styling with cn() + class-variance-authority variants
useWhen:
  ...
  - Building loading/empty/error states for data-backed screens
  - Implementing form UX with react-hook-form + zod validation
  - Making a layout RTL-ready and localizable with react-intl
  - Improving keyboard, focus, and screen-reader interaction on an existing web UI
decisionFramework:
  - condition: "Interactive control (button, menu, dialog, combobox) needs to be built from scratch"
    action: "Compose the shadcn/Radix primitive instead; never hand-roll ARIA/focus/keyboard state on raw divs"
  - condition: "Icon-only button or control has no visible text label"
    action: "Add an aria-label (or sr-only text); Radix cannot infer an icon's meaning"
  - condition: "Layout must adapt to the width of its own container, not the viewport (e.g. a card reused in a sidebar and a full-width grid)"
    action: "Use a container query (@container) rather than a viewport media query"
  - condition: "Any element's padding/margin/border/text-align depends on reading direction"
    action: "Use logical utilities (ps-*, pe-*, ms-*, me-*, text-start/end, border-s/e) never physical left/right, so it flips under dir=rtl"
  - condition: "A screen renders data fetched over the network"
    action: "Design all four states — loading (skeleton matching final layout), empty (illustration + message + CTA), error (retry affordance), success — never only the happy path"
  - condition: "Building a form with more than one field or any validation"
    action: "Use react-hook-form + zodResolver; show inline field errors wired with aria-describedby and aria-invalid, never an alert() or a toast for field-level validation"
  - condition: "A loading spinner would replace content that has a known shape"
    action: "Use a skeleton that mirrors the final layout instead of a centered spinner, to avoid layout shift and reduce perceived latency"
  - condition: "User-facing string is written directly in JSX"
    action: "Wrap it in a react-intl <FormattedMessage>/formatMessage with a stable id and add it to every locale catalog, never ship a hardcoded English literal"
  - condition: "A color is used to convey status (success/error/warning)"
    action: "Pair color with an icon and/or text; color alone fails colorblind users and WCAG 1.4.1"
  - condition: "Custom variant colors are added to a cva declaration"
    action: "Verify each foreground/background pair meets WCAG AA contrast (4.5:1 body, 3:1 large/UI); Radix guarantees semantics, not your palette's contrast"
  - condition: "A component hardcodes a hex/oklch color or a raw px font size"
    action: "Use a Tailwind semantic token (bg-primary, text-muted-foreground, text-sm) mapped to a CSS variable so theming and dark mode stay a data change, not a code change"
  - condition: "A modal/popover is rendered inside a container with overflow:hidden or a lower stacking context"
    action: "Rely on the primitive's Portal (default in shadcn) so it escapes the clip; do not re-parent it into the overflow container"
  - condition: "An interactive target is smaller than 24x24 CSS px"
    action: "Increase the hit area to at least 24x24 (WCAG 2.2 target size), preferably 44x44 for primary touch targets"
---
