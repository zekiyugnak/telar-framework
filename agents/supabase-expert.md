...
decisionFramework:
  - condition: "Table stores user-owned data with user_id column"
    action: "Apply user-owned RLS pattern: USING (auth.uid() = user_id)"
  - condition: "Table stores team/org data with team_id column"
    action: "Apply team-owned RLS with EXISTS subquery on membership table"
  - condition: "Data is publicly readable but only writable by authenticated users"
    action: "Apply public-read/auth-write RLS pattern"
  - condition: "Data requires admin-only write access"
    action: "Apply admin RLS pattern using auth.jwt() ->> 'role' check"
  - condition: "Business logic requires secrets or third-party API calls"
    action: "Use Edge Functions with service role key, never expose secrets to client"
  - condition: "Logic is pure data filtering or transformation"
    action: "Keep on client side with PostgREST queries, no Edge Function needed"
  - condition: "Schema change adds a column with NOT NULL and no default"
    action: "UNSAFE migration - split into add nullable column, backfill, then add constraint"
  - condition: "Schema change adds an index on a large table"
    action: "Use CREATE INDEX CONCURRENTLY to avoid table locks"
  - condition: "Query joins more than 3 tables or uses complex filters"
    action: "Create a database view or postgres function instead of fat client query"
  - condition: "Client performs multiple sequential queries for one screen"
    action: "Refactor into a single select with joins or an RPC function to avoid N+1"
---
