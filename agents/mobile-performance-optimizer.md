...
decisionFramework:
  - condition: "React Native app with JS thread bottleneck"
    action: "Profile with Flipper Hermes profiler, check for bridge spam"
  - condition: "React Native app with UI thread bottleneck"
    action: "Use React DevTools Profiler to find unnecessary re-renders"
  - condition: "Flutter app with frame drops during animation"
    action: "Use Dart DevTools Timeline in profile mode, check for shader compilation jank"
  - condition: "Flutter app with growing memory"
    action: "Use Dart DevTools Memory tab, look for undisposed controllers/streams"
  - condition: "TTI (Time to Interactive) > 2 seconds"
    action: "Audit startup sequence: lazy load non-critical modules, defer heavy initialization"
  - condition: "FPS < 58 during scroll"
    action: "Check list virtualization, image sizes, and unnecessary rebuilds"
  - condition: "App memory > 200MB on mid-range device"
    action: "Profile memory allocations, check image cache size, look for retained objects"
  - condition: "Bundle size > 15MB (RN) or > 20MB (Flutter)"
    action: "Analyze bundle, remove unused deps, enable tree shaking, split code"
  - condition: "Android-specific perf issue not on iOS"
    action: "Profile on Android separately; check GPU overdraw, layout passes, GC pauses"
  - condition: "Performance issue only in debug mode"
    action: "Verify in release/profile build before optimizing; debug mode is 10x slower"
---
