...
decisionFramework:
  - condition: "New Flutter project with simple to moderate state needs"
    action: "Use Riverpod with code generation (@riverpod annotation)"
  - condition: "Enterprise project with strict architecture requirements"
    action: "Use Bloc with freezed for events/states"
  - condition: "Need to call a native API once and get a result"
    action: "Use MethodChannel"
  - condition: "Need continuous data stream from native side (sensors, location)"
    action: "Use EventChannel"
  - condition: "Need high-performance native interop (image processing, crypto)"
    action: "Use dart:ffi with C/C++ library"
  - condition: "Dart 3+ project with union types needed"
    action: "Use sealed classes instead of freezed unions"
  - condition: "Data class with multiple return values"
    action: "Use Dart 3 records for lightweight tuples"
  - condition: "Primitive value keeps being confused with another (UserId vs OrderId, Email vs String)"
    action: "Wrap with a Dart 3 extension type — zero runtime cost, real compile-time identity"
  - condition: "Defining a port / protocol that tests need to implement but production should not extend"
    action: "Use abstract interface class — forbids extends, requires implements"
  - condition: "Defining a concrete domain type with a stable API surface"
    action: "Use final class — closes the type against subclassing and implementation"
  - condition: "New feature module with remote data + local cache"
    action: "Define a repository interface in domain/, implement in data/, inject through a provider — wrap DioException into typed domain errors at the boundary"
  - condition: "App needs dev/staging/prod environments"
    action: "Configure Flutter flavors with separate Firebase configs per flavor"
  - condition: "Widget tree deeper than 5 levels in single build method"
    action: "Extract into smaller composable widgets"
---
