---
id: rust-sqlx-patterns
category: skill
impact: CRITICAL
impactDescription: "Prevents connection-pool exhaustion outages and N+1 query storms; catches SQL/schema drift at compile time instead of in production"
tags: [rust, sqlx, postgres, connection-pool, migrations, transactions, multi-tenant, ci]
capabilities:
  - Writing compile-time verified queries with query!/query_as! against a live or offline schema
  - Choosing between compile-time macros and the runtime query_as::<_, T> API
  - Sizing and configuring a PgPool for production load
  - Managing schema changes with sqlx migrate
  - Using transactions correctly, including rollback-on-error semantics
  - Avoiding N+1 query patterns with joins or batched IN-queries
  - Running CI builds and tests without a live database via .sqlx offline mode
  - Scoping queries per tenant explicitly when the service holds elevated DB privileges
useWhen:
  - Writing or reviewing sqlx queries in a Rust service
  - Setting up or tuning a PgPool for a new or existing service
  - Adding or reviewing a schema migration
  - A handler is issuing one query per row inside a loop (N+1 suspicion)
  - Setting up CI to build/test without provisioning a live Postgres instance
  - The service is multi-tenant and needs to guarantee query-level tenant isolation despite running with elevated DB privileges
---
