...
category: skill
impact: HIGH
impactDescription: "Encodes stable locator + no-fixed-sleep waiting strategies for shadcn/Radix, TanStack Table/Query, and react-hook-form/zod, so generated E2E specs survive UI churn and async data settling without flaky sleeps"
tags: [testing, e2e, playwright, locators, shadcn, tanstack, web]
capabilities:
  - data-testid-primary + getByRole/getByLabel locator strategy
  - shadcn/Radix combobox, dialog, toast locator recipes
  - TanStack Table sort/filter/paginate assertions on rendered rows
  - TanStack Query async settling via web-first auto-retrying assertions
  - react-hook-form + zod validation-state assertions (aria-invalid/error text)
useWhen:
  - Writing Playwright locators for a shadcn/Refine/TanStack app
  - A spec is flaky because it waits on timeouts instead of UI state
---
