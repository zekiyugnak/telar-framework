...
category: skill
impact: HIGH
impactDescription: Prevents fix-churn by enforcing root cause investigation before any code changes
tags: [debugging, root-cause, investigation, troubleshooting, mobile-debugging, error-analysis]
capabilities:
  - 4-phase root cause investigation methodology
  - Platform-specific log gathering (Metro, Xcode, ADB logcat)
  - Pattern analysis against working codebase examples
  - Hypothesis testing with single-variable changes
  - Escalation protocol after repeated fix failures
useWhen:
  - A test fails or becomes flaky
  - A build error or runtime crash occurs
  - A feature works on one platform but not the other
  - A bug report is received without clear cause
