...
category: skill
impact: CRITICAL
impactDescription: "Eliminates rebuild storms from setState in StatefulWidgets, reducing widget rebuilds by 10-50x in complex screens"
tags: [riverpod, bloc, state-management, async-notifier, state-notifier, provider-scope, family]
capabilities:
  - Riverpod vs BLoC decision framework with clear criteria
  - AsyncNotifier patterns for data fetching
  - StateNotifier patterns for complex state machines
  - Provider scoping and family modifiers
  - Eliminating unnecessary widget rebuilds
useWhen:
  - Choosing state management for Flutter
  - Fixing excessive widget rebuilds
  - Implementing Riverpod AsyncNotifier or StateNotifier
  - Managing complex app state with proper scoping
  - Refactoring from setState to scalable state management
---
