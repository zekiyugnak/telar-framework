...
category: skill
tags: [networking, http, dio, rest, interceptors, retry, multipart, cancellation]
capabilities:
  - http vs dio decision framework
  - Typed REST client with error mapping
  - Auth interceptor with token-refresh race protection
  - Exponential-backoff retry interceptor
  - Multipart upload with progress
  - Request cancellation on route/widget dispose
  - Testing with MockClient and DioAdapter
useWhen:
  - Choosing between http and dio for a new Flutter app
  - Adding auth interceptor with refresh token handling
  - Retrying failed requests with backoff
  - Uploading files with progress reporting
  - Canceling in-flight requests when a widget disposes
