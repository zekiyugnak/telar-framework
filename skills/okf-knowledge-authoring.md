---
id: okf-knowledge-authoring
category: skill
impact: HIGH
impactDescription: "Keeps the docs/knowledge/ OKF bundle trustworthy — a wrong or uncited concept silently misleads every downstream agent, so authoring must enforce the format contract, one-way authority, and PII boundary on every edit."
tags: [okf, knowledge-bundle, documentation, citations, pii, cross-linking]
capabilities:
  - Author an OKF concept file with valid frontmatter (required non-empty type) and recommended keys
  - Cite each concept's authority under a # Citations heading (ADR / EDR / schema)
  - Apply one-way authority — the bundle derives from docs/adr + docs/data_model and never originates a decision
  - Enforce the PII/security boundary (schema/semantics/metadata only; no rows, secrets, service_role)
  - Cross-link concepts as a graph via ordinary markdown links (path = entry name)
  - Maintain source-first — land the source change, then derive the concept, then log it
useWhen:
  - Writing or updating any concept in a docs/knowledge/ OKF bundle
  - A source-owning agent updates a table/RPC/enum and must update its derived concept in the same change
  - The okf-knowledge-curator runs bulk production, drift lint, or cross-linking
  - Reviewing a concept for frontmatter / citation / PII conformance
---
