---
id: nextjs-auth-app-router
category: skill
impact: CRITICAL
impactDescription: "Prevents unauthorized data access from stale-cookie/session-desync bugs that a middleware-only auth check cannot catch"
tags: [nextjs, app-router, authentication, session, middleware, supabase]
capabilities:
  - Gating routes with a fast, edge-level session check in middleware.ts
  - Re-verifying sessions as the true authorization boundary in Server Components and Route Handlers
  - Passing authenticated user context down the Server Component tree without client-side Context
  - Configuring Supabase's @supabase/ssr cookie-based session helpers for browser, server, and middleware contexts
  - Distinguishing this web/cookie-session flow from the mobile PKCE flow covered elsewhere in this plugin
useWhen:
  - Setting up login-gated routes in a Next.js App Router console
  - Deciding whether an auth check belongs in middleware, a Server Component, or a Route Handler
  - Debugging why an unauthenticated or session-desynced request still reached protected data
  - Configuring Supabase auth for a Next.js web app (as opposed to a React Native/Flutter mobile app)
  - Reviewing whether a Server Component correctly re-verifies auth instead of trusting a cached session
---
