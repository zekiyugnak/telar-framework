---
id: tanstack-router-patterns
category: skill
impact: HIGH
impactDescription: "Eliminates an entire class of route/query waterfalls and unguarded-route bugs by making routing type-safe end to end"
tags: [tanstack-router, react, routing, type-safety, code-splitting, admin-panel, web]
capabilities:
  - File-based route definitions with generated, fully type-safe route trees
  - Type-safe search params with runtime validation
  - Route loaders combined with pending/error boundary components
  - Per-route code-splitting without manual React.lazy wiring
  - Protected-route guards via beforeLoad that run before any UI renders
useWhen:
  - Setting up routing for a new Vite + React admin panel
  - Adding a new screen that needs its own deep-linkable URL
  - A route needs query-string state (filters, page, sort) that survives reload
  - Gating a route behind an authenticated session before it renders
  - Debugging a route that flashes unauthenticated content before redirecting
---
