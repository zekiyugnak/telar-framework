---
id: web-design-system-tokens
category: skill
impact: HIGH
impactDescription: One MASTER token source generates Tailwind v4 @theme + shadcn CSS variables for every web surface AND Flutter Dart, so marketing/client/admin and mobile cannot drift and a rebrand is a one-line edit
tags: [design-system, tokens, tailwind, css-variables, theming, dark-mode, web]
capabilities:
  - Global MASTER token source authored once as JSON, brand- and platform-neutral
  - Tailwind v4 @theme block wired to :root/.dark CSS custom properties (shadcn two-layer model)
  - Semantic token layering (primitive → semantic → component) with a strict downward reference rule
  - Per-surface and per-vertical overrides with a deterministic cascade precedence chain
  - Explicit light/dark palettes with a build-time contrast gate
  - Single-source codegen that emits web CSS and Flutter-consumable Dart together
useWhen:
  - Setting up a design system for a Tailwind v4 + shadcn/Radix web codebase
  - One token set must feed multiple web surfaces (marketing, client, admin) without drifting
  - Adding whitelabel or multi-vertical theming on top of a shared base palette
  - Sharing one design source across both web and Flutter so the platforms cannot diverge
  - Translating a MASTER token file into runtime CSS variables and utilities
---
