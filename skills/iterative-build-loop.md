---
id: iterative-build-loop
category: skill
impact: MEDIUM
impactDescription: Enables multi-session feature builds with progress persistence, preventing context loss between Claude Code sessions
tags: [baton, iterative, build-loop, next-step, progress-tracking, multi-session, feature-build]
capabilities:
  - Multi-step feature building with baton file handoffs
  - Progress tracking with completion percentage
  - Build-verify-document cycle for each iteration
  - Simulator verification between build steps
  - Context preservation across sessions via next-step.md
useWhen:
  - Building a feature too large for a single session
  - Need to verify on simulator between build steps
  - Working iteratively with design feedback between sessions
  - Multi-screen feature that should be built incrementally
---
