---
id: refine-admin-patterns
category: skill
impact: HIGH
impactDescription: "Prevents re-implementing CRUD, routing, and auth by hand on top of Refine — and prevents the anon-key client from treating Refine's client-side access control as a security boundary instead of RLS"
tags: [refine, refinedev, admin-panel, supabase, data-provider, auth-provider, react, web]
capabilities:
  - Deriving list/create/edit/show routes from a Refine `resources` array instead of hand-wiring each CRUD screen
  - Wiring a Supabase `dataProvider` (anon key + RLS) with server-side pagination, sorting, and filtering via `useTable`/`useList`
  - Wiring a Supabase-Auth `authProvider` (check/login/logout/getIdentity/onError) as Refine's single session source
  - Mapping an `accessControlProvider` to Postgres roles as UI-hiding only, with RLS as the real boundary
  - Rendering Refine's `useTable` through TanStack Table headless UI + shadcn/Radix primitives
  - Bridging Refine's `i18nProvider` onto an existing react-intl catalog instead of running two i18n systems
useWhen:
  - Building or extending a Refine admin resource (a new table, its create/edit/show screens) on Supabase
  - Wiring `dataProvider` / `authProvider` for a Vite + React 19 + Refine panel that only holds the anon key
  - Deciding how `accessControlProvider` relates to RLS and whether a role check is security or just UX
  - Integrating Refine's data hooks with a TanStack Table + shadcn data grid you already have
  - Adding react-intl-backed translations to Refine's framework strings (buttons, notifications, titles)
---
