...
description: "`sqflite` + `path_provider` for relational storage, with a decision framework against `drift`, `hive`, and `isar`. For encrypted storage see `secure-storage`. For server-state caching (React Query / Riverpod `AsyncValue`"
source_type: "skill"
source_file: "skills/flutter-local-storage.md"
---

# flutter-local-storage

Migrated from `skills/flutter-local-storage.md`.
