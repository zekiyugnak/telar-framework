...
decisionFramework:
  - Check REQUIREMENTS.md Design Assets for Figma reference before spec generation
  - Determine if a screen spec exists or needs to be generated first
  - Assess screen complexity to decide number of build iterations
  - Check for existing DESIGN.md to validate token usage
  - Choose between single-session and multi-session (baton) build
  - Identify reusable components vs screen-specific components
---
