...
decisionFramework:
  - condition: "Team is starting a new HTTP service and has no existing actix-web investment"
    action: "Use axum. It shares tokio/hyper/tower with the rest of the async ecosystem, has a smaller macro surface, and its extractor model composes cleanly with tower middleware."
  - condition: "Team already has a large actix-web codebase with actor-based patterns in use"
    action: "Stay on actix-web for that service; do not force a rewrite. Use axum only for new, separate services."
  - condition: "Queries are known at compile time and the schema is stable"
    action: "Use sqlx with query!/query_as! macros for compile-time verified SQL against a real or offline (.sqlx) schema cache."
  - condition: "The service needs a full ORM with generated migrations, associations, and query builder DSL"
    action: "Consider diesel instead of sqlx, but only if the team values compile-time schema modeling over raw SQL control. Default to sqlx unless this is a strong requirement."
  - condition: "A route needs to know who the caller is before the handler body runs"
    action: "Implement a custom extractor (FromRequestParts) for the identity type. Do not parse headers manually inside every handler."
  - condition: "A cross-cutting concern applies to many/all routes (logging, CORS, rate limiting, timeouts)"
    action: "Implement it as a tower::Layer/middleware on the router, not as per-handler boilerplate."
  - condition: "Expected concurrent DB-bound requests at peak load"
    action: "Size the sqlx PgPool max_connections to roughly (peak concurrent DB-bound requests), not (total expected RPS); start at 10-20 per instance and tune from pool-wait metrics, not guesswork."
  - condition: "A handler needs to return errors from multiple layers (validation, DB, external API)"
    action: "Define a single AppError enum with thiserror, implement IntoResponse once, and convert layer-specific errors into it with `?` and `From` impls."
  - condition: "Work must happen after a request returns but before the service can shut down safely (e.g. sending an email)"
    action: "Spawn a tokio::task for lightweight, best-effort background work; use an external queue (e.g. a jobs table polled by a worker, or SQS/Redis) when work must survive process restarts or needs retries."
  - condition: "Handlers are directly embedding SQL and business rules together and the file is growing past ~300 lines"
    action: "Introduce a service layer (business rules) and a repository layer (SQL) between the handler and the database; keep the split only where it earns its complexity (see rust-service-architecture skill)."
  - condition: "The service is deployed to Kubernetes or similar orchestrator"
    action: "Implement SIGTERM-aware graceful shutdown with axum::serve's with_graceful_shutdown and expose a /healthz endpoint that reflects real dependency health, not just process liveness."
---
