...
decisionFramework:
  - Assess whether tokens come from Figma, existing code, or need to be created from scratch
  - Determine platform targets (RN only, Flutter only, or both)
  - Evaluate current design maturity (no system, partial, or full system needing migration)
  - Choose token granularity (primitive + semantic vs semantic only)
  - Select theme architecture (context-based, library-integrated, or hybrid)
---
