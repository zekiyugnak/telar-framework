...
decisionFramework:
  - condition: "Binary size and memory footprint are primary constraints and the team is comfortable with Rust"
    action: "Choose Tauri — ships a ~5 MB installer vs ~100+ MB for Electron; uses the OS webview (WKWebView/WebView2/WebKitGTK) and a Rust backend, eliminating the bundled Chromium/Node"
  - condition: "The team is JS-only or the app must embed Node.js APIs (fs, child_process, native npm modules) directly"
    action: "Choose Electron — the full Node.js runtime is available in the main process and native npm modules compile against it without a Rust FFI layer"
  - condition: "The app targets Windows 7/8 or Linux distros with an outdated system WebView"
    action: "Choose Electron — it bundles its own Chromium so rendering is consistent regardless of OS WebView version; Tauri's WebView2 requires a runtime install on older Windows"
  - condition: "contextIsolation is false OR nodeIntegration is true in any BrowserWindow"
    action: "Enable contextIsolation:true and nodeIntegration:false immediately; refactor renderer-side code to use the preload contextBridge API — this is a critical security boundary, not optional"
  - condition: "A renderer needs to call a main-process capability (file I/O, OS dialog, IPC to another window)"
    action: "Electron: expose a named method via contextBridge in preload.ts; validate all arguments in the handler before acting. Tauri: define a #[tauri::command] function, add it to the capability JSON — never expose broader permission than the command requires"
  - condition: "Auto-update must be delivered from a self-hosted server"
    action: "Electron: use electron-updater with a private S3/GCS bucket and a RELEASES file; verify the update signature before applying. Tauri: configure the updater with a pinned pubkey in tauri.conf.json; reject any update whose signature does not verify against that key"
  - condition: "A deep link or custom protocol (myapp://) arrives with user-controlled data"
    action: "Validate and sanitize the URL in the main process before passing any part of it to renderer or using it as a file path — protocol handlers are a common injection vector"
  - condition: "The renderer needs the full React/TS web frontend from an existing web project"
    action: "Point Electron's loadURL / Tauri's devUrl at the Vite dev server in development and at the built dist/ in production; apply the same CSP headers in the webPreferences that the web server would set"
---
