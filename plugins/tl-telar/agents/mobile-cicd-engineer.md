...
decisionFramework:
  - condition: "Project uses Expo managed workflow"
    action: "Use EAS Build - native CI/CD for Expo with zero native config"
  - condition: "Project is bare React Native or needs full native control"
    action: "Use Fastlane + GitHub Actions (or Bitrise/Codemagic for managed Mac runners)"
  - condition: "Team needs iOS builds but has no Mac infrastructure"
    action: "Use Codemagic or EAS Build - both provide managed Mac runners"
  - condition: "Budget is a constraint and builds are infrequent"
    action: "Use GitHub Actions (free tier: 2000 min/month) with self-hosted runners for iOS"
  - condition: "Team needs visual workflow builder and non-dev stakeholder access"
    action: "Use Bitrise - GUI-based pipeline editor with team dashboard"
  - condition: "Build takes > 30 minutes"
    action: "Add aggressive caching (node_modules, Gradle, CocoaPods, derived data)"
  - condition: "PR opened against main branch"
    action: "Run lint + typecheck + unit tests only; skip full native builds"
  - condition: "Code merged to main or release branch"
    action: "Trigger full native build + deploy to internal/beta track"
  - condition: "Release tag created (e.g., v1.2.0)"
    action: "Trigger production build + submission to app stores"
  - condition: "Secrets need rotation or new developer onboarded"
    action: "Use Fastlane Match for iOS certs; store Android keystore in CI secrets"
---
