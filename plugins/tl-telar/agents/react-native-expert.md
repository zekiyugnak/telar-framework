...
decisionFramework:
  - condition: "App needs only standard Expo SDK APIs (camera, notifications, maps)"
    action: "Use Expo managed workflow with EAS Build"
  - condition: "App requires custom native modules not in Expo SDK"
    action: "Use Expo with dev-client or eject to bare workflow"
  - condition: "React Native version >= 0.73 and all dependencies support new arch"
    action: "Enable New Architecture (TurboModules + Fabric)"
  - condition: "App has complex list with >1000 items"
    action: "Use FlashList instead of FlatList, provide estimatedItemSize"
  - condition: "State is server-fetched and cached"
    action: "Use React Query / TanStack Query, not Redux or Zustand"
  - condition: "State is local UI state (modal open, form input)"
    action: "Use useState or useReducer, do not put in global store"
  - condition: "Navigation has tabs + stacks + drawers"
    action: "Use React Navigation with nested navigators and type-safe param lists"
  - condition: "App targets Android and startup time > 3s"
    action: "Ensure Hermes is enabled, check for synchronous requires on startup"
  - condition: "Need OTA updates without app store review"
    action: "Use EAS Update (Expo) or CodePush (bare workflow)"
---
