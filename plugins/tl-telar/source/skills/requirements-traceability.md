...
category: skill
impact: MEDIUM
impactDescription: Links every requirement to its implementation tasks, test cases, and source files — preventing coverage gaps and making change impact analysis instant
tags: [traceability, requirements, rtm, coverage, testing, change-management]
capabilities:
  - Build a Requirements Traceability Matrix (RTM) from REQUIREMENTS.md + PLAN.md
  - Forward traceability: requirement → tasks → files → tests
  - Backward traceability: file/test → requirement
  - Coverage gap detection (requirements with no tasks or tests)
  - Impact analysis: which requirements are affected by a file change
useWhen:
  - After PLAN.md is complete and implementation begins
  - Before a release to verify all requirements are covered
  - When a requirement changes to assess cascade impact
  - When reviewing code to check spec compliance
---
