...
category: skill
impact: HIGH
impactDescription: "Missing or inconsistent SEO/OG tags silently kill organic search visibility and make shared job/career links render as blank or wrong-title cards on LinkedIn, Slack, and X — directly reducing candidate applications"
tags: [astro, seo, open-graph, meta-tags, sitemap, structured-data]
capabilities:
  - Reusable SEO/Layout component generating title, description, and canonical tags
  - Open Graph and Twitter Card metadata wired from a single source of truth
  - JSON-LD structured data for Organization and JobPosting schema
  - Dynamic and static Open Graph image generation strategies
  - Sitemap and robots.txt integration with multi-locale routing
useWhen:
  - Setting up SEO metadata for a new Astro marketing or careers site
  - Auditing why a page's link preview shows the wrong title, image, or description
  - Adding job postings, blog posts, or landing pages that each need correct OG tags
  - Debugging duplicate-content or canonical URL issues across locales
  - Implementing structured data so job postings appear in Google's job search results
---
