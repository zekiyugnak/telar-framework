...
category: skill
impact: CRITICAL
impactDescription: "Prevents random logouts from token refresh race conditions affecting 15-30% of active sessions in production"
tags: [supabase, authentication, pkce, social-auth, token-refresh, session, google, apple, oauth]
capabilities:
  - PKCE flow implementation for React Native and Flutter
  - Token refresh race condition prevention
  - Social auth (Google, Apple, GitHub) with platform-specific setup
  - Session persistence and recovery patterns
  - Auth state change handling
useWhen:
  - Setting up Supabase authentication in mobile apps
  - Debugging random user logouts
  - Implementing social login (Google, Apple, GitHub)
  - Handling token refresh edge cases
  - Managing auth sessions across app restarts and background states
---
