---
id: networking-patterns
category: skill
impact: HIGH
impactDescription: "Eliminate race conditions, duplicate API calls, and network-related crashes with production-grade patterns"
tags: [networking, retry, deduplication, optimistic-update, offline-queue, certificate-pinning, caching]
capabilities:
  - Implement retry strategies with exponential backoff and jitter
  - Deduplicate concurrent identical requests
  - Apply optimistic updates with automatic rollback on failure
  - Queue requests offline and sync when connectivity returns
  - Configure certificate pinning for API security
  - Design effective API response caching strategies
useWhen:
  - Duplicate API calls fire on fast navigation or double-taps
  - Network errors crash the app or show cryptic messages
  - Users on flaky connections see inconsistent data
  - Need to support offline-first functionality
  - Securing API communication against MITM attacks
---
