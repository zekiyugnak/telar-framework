---
id: tremor-dashboard-charts
category: skill
impact: MEDIUM
impactDescription: "Cuts dashboard build time by reusing pre-built, accessible chart primitives instead of hand-rolling SVG chart code per KPI"
tags: [tremor, charts, dashboard, kpi, data-visualization, shadcn, admin-panel, web]
capabilities:
  - Choosing Tremor for standard admin-dashboard chart and KPI-card needs
  - Recognizing when a custom charting approach is actually warranted instead
  - Aligning Tremor's visual output with shadcn/ui design tokens
  - Building responsive chart containers that don't overflow or clip
  - Shaping Supabase query results into Tremor's expected data formats
useWhen:
  - Building KPI cards or trend charts for an admin/operator dashboard
  - Deciding whether Tremor or a lower-level charting library fits a given visualization
  - A Tremor chart renders blank, mis-scaled, or visually inconsistent with the rest of the UI
  - Shaping a Supabase aggregate query's result for a line, bar, or donut chart
---
