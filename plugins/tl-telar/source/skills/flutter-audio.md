...
id: flutter-audio
category: skill
tags: [audio, just-audio, audio-service, background-playback, playlist, streaming]
capabilities:
  - just_audio playback with play/pause/seek/stop
  - Playlist (ConcatenatingAudioSource) and queue management
  - Background playback via audio_service
  - Audio focus, interruption, and headphone-unplug handling
  - Streaming from URL with caching
  - iOS Info.plist and Android manifest setup for background audio
  - Testing audio logic without hitting a real player
useWhen:
  - Adding audio playback to a Flutter app
  - Building a podcast, audiobook, or music-player feature
  - Handling interruptions from calls or other audio apps
  - Playing audio in the background with a lock-screen control
  - Debugging audio that stops when the screen locks
---
