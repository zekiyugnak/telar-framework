...
category: skill
impact: CRITICAL
impactDescription: "Prevents user data leaks on logout, eliminates stale screen state bugs affecting 40%+ of navigation-heavy apps"
tags: [react-navigation, deep-linking, stack, tabs, drawer, routing, auth-flow, modal, typescript]
capabilities:
  - Auth flow patterns switching between auth and main stacks based on token
  - Modal stacks with presentation modes
  - Tab persistence and state saving
  - Navigation reset patterns for logout
  - Type-safe navigation with TypeScript
  - Deep linking with universal links
useWhen:
  - Setting up navigation in React Native apps
  - Implementing auth flow with protected routes
  - Adding deep linking or universal links
  - Debugging navigation state not resetting on logout
  - Creating modal presentation flows
---
