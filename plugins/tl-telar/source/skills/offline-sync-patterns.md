---
id: offline-sync-patterns
category: skill
impact: HIGH
impactDescription: "Prevents data loss when users edit offline; correct conflict resolution preserves 100% of user changes"
tags: [offline, sync, conflict-resolution, local-first, watermelondb, sqlite, queue]
capabilities:
  - Implement conflict resolution strategies (last-write-wins, field-level merge, manual)
  - Build queue-based sync with retry and exponential backoff
  - Design local-first architecture with WatermelonDB or SQLite
  - Display sync status indicators for user feedback
useWhen:
  - Building apps that must work without network connectivity
  - Implementing data synchronization between local and remote databases
  - Handling merge conflicts when multiple devices edit the same data
  - Designing offline-first mobile architectures
---
