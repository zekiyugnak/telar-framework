...
category: skill
impact: HIGH
impactDescription: "Prevent app crashes from reaching users and convert unhandled exceptions into recoverable states"
tags: [error-handling, error-boundaries, crash-reporting, graceful-degradation, sentry, crashlytics]
capabilities:
  - Implement React Native Error Boundaries with fallback UI
  - Configure Flutter error handling with FlutterError.onError and PlatformDispatcher
  - Integrate crash reporting with Sentry and Crashlytics
  - Classify network errors and show user-friendly messages
  - Apply graceful degradation patterns for partial failures
useWhen:
  - Unhandled promise rejections crash the entire app
  - Users see white screens or cryptic error messages
  - Crash reports show errors that could have been caught
  - Need to add crash reporting to a mobile app
  - Network failures cause poor user experience
---
