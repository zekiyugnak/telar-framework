...
category: skill
impact: HIGH
impactDescription: "Keeps services testable and mergeable as they grow past a handful of endpoints; prevents config/secrets sprawl and breaking API changes for mobile clients"
tags: [rust, architecture, layering, dependency-injection, config, error-handling, workspace, api-versioning]
capabilities:
  - Layering a service into handler, service, and repository responsibilities
  - Sharing dependencies via AppState and Arc<T> without over-engineering DI
  - Managing configuration with the config/envy crates plus .env for local dev
  - Designing a structured error hierarchy with thiserror
  - Deciding when to split a single crate into a Cargo workspace
  - Choosing and implementing an API versioning strategy that doesn't break mobile clients
useWhen:
  - Deciding how to organize a new or growing Rust service's modules
  - A handler file is mixing HTTP concerns, business rules, and raw SQL in one function
  - Introducing typed configuration instead of scattered std::env::var calls
  - The codebase has grown enough to consider splitting into multiple crates
  - Planning a breaking API change that must not break already-shipped mobile app versions
---
