...
category: skill
impact: CRITICAL
impactDescription: "Unencrypted token storage allows any rooted/jailbroken device or malware to steal auth credentials"
tags: [keychain, keystore, mmkv, encryption, biometric, secure-storage, credentials]
capabilities:
  - Choose correct storage backend per platform (Keychain, Keystore, MMKV)
  - Implement biometric gating for sensitive data access
  - Compare expo-secure-store vs react-native-keychain vs flutter_secure_storage
  - Encrypt data at rest with hardware-backed keys
useWhen:
  - Storing authentication tokens or refresh tokens
  - Saving API keys or encryption keys on device
  - Implementing biometric-protected data access
  - Choosing between secure storage libraries
