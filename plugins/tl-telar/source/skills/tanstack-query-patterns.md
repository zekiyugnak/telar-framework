...
category: skill
impact: HIGH
impactDescription: "Removes duplicate/stale-data bugs from ad hoc cache keys and closes the gap between route loaders and component reads"
tags: [tanstack-query, react-query, supabase, caching, realtime, admin-panel, web]
capabilities:
  - Query key factories that keep cache entries addressable and invalidation precise
  - Wiring TanStack Query into TanStack Router loaders via ensureQueryData
  - Optimistic updates with correct rollback on mutation failure
  - Targeted cache invalidation strategy after mutations
  - Using Supabase Realtime subscriptions to invalidate/patch the query cache
useWhen:
  - Deciding how to structure query keys for a growing admin panel
  - Wiring a TanStack Router loader and a component to share one cache entry
  - Implementing an edit action that should feel instant (optimistic update)
  - A mutation succeeds but the UI shows stale data until a manual refresh
  - Multiple operators viewing the same table need to see each other's changes live
---
