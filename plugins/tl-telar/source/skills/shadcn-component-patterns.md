---
id: shadcn-component-patterns
category: skill
impact: HIGH
impactDescription: "Cuts custom interactive-component build time significantly and eliminates a whole class of keyboard/focus/ARIA accessibility bugs by inheriting Radix's tested primitive behavior"
tags: [shadcn, ui, radix, tailwind, components, design-system]
capabilities:
  - Adding and owning shadcn/ui components via the CLI instead of installing a UI library
  - Composing Radix-based primitives into compound, feature-level components
  - Applying cn() and class-variance-authority (cva) for consistent variant styling
  - Theming components through CSS variables shared with Tailwind's token system
  - Building accessible dropdowns, dialogs, and table toolbars without hand-rolling ARIA
  - Wiring a dark mode toggle that persists and avoids theme flash
useWhen:
  - Adding a new interactive UI element (dropdown, dialog, popover, form field) to a console
  - Reviewing whether a hand-rolled component should instead be a shadcn primitive
  - Standardizing button/badge/input variants across a dashboard
  - Setting up or auditing dark mode support
  - Building a composite component like a data table toolbar or filter bar
---
