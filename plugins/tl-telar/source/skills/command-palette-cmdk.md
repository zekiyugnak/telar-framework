...
category: skill
impact: MEDIUM
impactDescription: "Cuts navigation and repeat-action time for power-user operators who default to keyboard over mouse in a dense admin panel"
tags: [cmdk, command-palette, keyboard-navigation, accessibility, admin-panel, web]
capabilities:
  - Building a cmd-k style command palette with global keyboard activation
  - Guarding the global shortcut so it doesn't fire while typing in a form field
  - Fuzzy search and grouped results (navigation vs actions)
  - Arrow-key navigation with correct scroll-into-view behavior
  - Wiring palette navigation commands into TanStack Router
useWhen:
  - Adding a cmd-k / ctrl-k command palette to an admin panel's shell
  - Operators need a fast keyboard path to jump between screens or trigger actions
  - The global keyboard listener fires unexpectedly while a user is typing in a text field
  - Deciding what belongs in the command palette versus a dedicated button or menu
---
