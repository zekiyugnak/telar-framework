---
id: astro-content-performance
category: skill
impact: HIGH
impactDescription: "Unnecessary hydration and unoptimized hero images are the most common cause of poor LCP/CLS on marketing sites, directly hurting Core Web Vitals-based search ranking and candidate drop-off before the page even settles"
tags: [astro, islands, performance, hydration, core-web-vitals, image-optimization]
capabilities:
  - Choosing the correct client: hydration directive per component
  - Auditing a site for unnecessary or over-eager hydration
  - Responsive, format-aware image optimization with astro:assets
  - Type-safe structured content via Content Collections and Zod schemas
  - Diagnosing Core Web Vitals regressions (LCP, CLS) specific to Astro sites
useWhen:
  - A marketing or careers page ships more JavaScript than it needs
  - Lighthouse/PageSpeed flags poor LCP, CLS, or TBT on a static-content page
  - Deciding which client:* directive (or none) a new component should use
  - Modeling job listings, company pages, or blog posts as structured content
  - Reviewing a PR that adds a new interactive component to a static page
---
